...
from tkinter import ttk, messagebox, scrolledtext, simpledialog, Entry, Button, Frame, Label
import threading
import sys
import os
import time
...
class ETRCGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("ETRC - 增强型远程终端控制 v1.7 GUI")
    ...
    def create_console_tab(self):
        ...
        self.info_label = tk.Label(frame, textvariable=self.info_text, justify=tk.LEFT)
        ...
        self.activate_btn = tk.Button(btn_frame, text="激活程序", command=self.activate_program_gui)
        self.activate_btn.pack(side=tk.LEFT, padx=5)
        
        self.start_server_btn = tk.Button(btn_frame, text="启动被控模式", command=self.start_server_gui)
        self.start_server_btn.pack(side=tk.LEFT, padx=5)
        
        self.start_client_btn = tk.Button(btn_frame, text="启动主控模式", command=self.start_client_gui)
        self.start_client_btn.pack(side=tk.LEFT, padx=5)
        
        self.change_pass_btn = tk.Button(btn_frame, text="修改长期密码", command=self.change_password_gui)
        self.change_pass_btn.pack(side=tk.LEFT, padx=5)
        
        tk.Button(btn_frame, text="退出程序", command=self.on_close).pack(side=tk.RIGHT, padx=5)
        
        # 授权码输入区域 - 解锁配置管理
        auth_frame = ttk.LabelFrame(frame, text="配置管理授权")
        auth_frame.pack(fill=tk.X, padx=10, pady=10)
        
        tk.Label(auth_frame, text="输入授权码解锁配置管理:").pack(side=tk.LEFT, padx=(5, 0))
        
        self.auth_code_var = tk.StringVar()
        auth_entry = tk.Entry(auth_frame, textvariable=self.auth_code_var, width=25, show="*")
        auth_entry.pack(side=tk.LEFT, padx=5)
        
        tk.Button(auth_frame, text="解锁", command=self.unlock_config_tab).pack(side=tk.LEFT, padx=5)
    
    def update_info_text(self):
        """更新系统信息文本"""
        info = f"""ETRC - 增强型远程终端控制 v2.0
...
"""
    ...
    def check_activation_status(self):
        ...
        if not self.core.ACTIVATED:
            ...
            self.change_pass_btn.config(state=tk.DISABLED)
            ...
            for widget in self.client_tab.winfo_children():
                if isinstance(widget, (Button, Entry)):
                    widget.config(state=tk.DISABLED)
            
            messagebox.showwarning("未激活", "程序未激活，请先激活才能使用功能")
    
    def check_permanent_password(self):
        """检查长期密码，如果没有则弹出设置对话框"""
        if self.core.ACTIVATED and not self.core.PERMANENT_PASSWORD:
            self.set_permanent_password()
    
    def set_permanent_password(self):
        """设置长期密码对话框"""
        password_dialog = tk.Toplevel(self.root)
        password_dialog.title("设置长期密码")
        password_dialog.geometry("400x200")
        password_dialog.transient(self.root)
        password_dialog.grab_set()
        
        tk.Label(password_dialog, text="检测到尚未设置长期密码，请设置一个长期密码", 
                font=("Arial", 10)).pack(pady=10)
        
        tk.Label(password_dialog, text="新长期密码:").pack(pady=(5, 0))
        new_pass_var = tk.StringVar()
        tk.Entry(password_dialog, textvariable=new_pass_var, show="*", width=25).pack()
        
        tk.Label(password_dialog, text="确认新长期密码:").pack(pady=(5, 0))
        confirm_pass_var = tk.StringVar()
        tk.Entry(password_dialog, textvariable=confirm_pass_var, show="*", width=25).pack()
        ...
        def save_password():
            ...
            self.core.PERMANENT_PASSWORD = new_pass
            if self.core.save_config():
                self.update_info_text()
                messagebox.showinfo("成功", "长期密码设置成功")
                password_dialog.destroy()
            else:
                messagebox.showerror("错误", "保存密码失败")
        
        tk.Button(password_dialog, text="保存", command=save_password, width=10).pack(pady=15)
    
    def unlock_config_tab(self):
        """解锁配置管理标签页"""
        auth_code = self.auth_code_var.get()
        if auth_code == "#CONFIGMGR-AUTH-ROOT#":
            # 添加配置管理标签页
            self.notebook.add(self.config_tab, text="配置管理")
            self.create_config_tab()
            self.update_status("配置管理已解锁")
        else:
            messagebox.showerror("授权失败", "授权码错误，无法解锁配置管理")
    ...
    def activate_program_gui(self):
        ...
        def attempt_activation():
            ...
            if self.core.check_activation_key(self.core.ACTIVATION_CODE, key):
                ...
                success, error = self.core.save_config()  # 获取保存结果和错误信息
                if success:
                    messagebox.showinfo("激活成功", "程序已成功激活！")
                    activation_window.destroy()
                    self.update_info_text()
                    self.check_activation_status()  # 更新界面状态
                    self.check_permanent_password()  # 检查是否需要设置长期密码
                else:
                    # 显示具体的错误信息
                    messagebox.showerror("激活失败", f"保存激活状态失败: {error}")
            else:
...
class ETRCCore:
    ...
    def load_config(self):
        ...
        try:
            ...
            print(f"配置文件格式错误: {str(e)}，创建默认配置")
            self.create_default_config()
        except Exception as e:
            print(f"加载配置文件失败: {str(e)}，创建默认配置")
            self.create_default_config()
