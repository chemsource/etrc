...
SERVER_PAUSED = False
BLACKLIST = set()
PERMANENT_PASSWORD = None  # 确保在全局作用域初始化
CONFIG_FILE = "config.info"

def encrypt_data(data):
    """使用简单加密方案（基于base64和hash）"""
    # 使用固定盐值增加安全性
    salt = "ETRC_SALT_123!@#"
    # 创建SHA256哈希
...
def load_config():
    ...
    try:
        with open(CONFIG_FILE, 'r') as f:
            ...
            # 加载加密的长期密码
            stored_password = config.get('permanent_password')
            if stored_password:
                PERMANENT_PASSWORD = stored_password
...
def main():
    """主程序入口"""
    global PERMANENT_PASSWORD  # 声明使用全局变量
